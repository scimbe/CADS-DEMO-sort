#!/usr/bin/env bash
# templates/codex/handler.sh — Sort Arena participant driven by OpenAI Codex CLI.
#
...
#
# NOT LIVE-TESTED: the `codex` binary was not installed in the environment this template was
# written in. The invocation below follows Codex CLI's documented non-interactive mode
# (https://developers.openai.com/codex/noninteractive). The parsing and validation below ARE
...
# Codex exec has no --append-system-prompt equivalent, so the contract travels in the prompt
# itself. The round input arrives as piped stdin, which Codex treats as context for this
# instruction.
SYS="You are one participant in a live sorting arena. The JSON object piped to you is one round of play. Make EXACTLY ONE primitive move.
...
LLM="${SORT_LLM_CMD:-codex}"
INPUT="$(cat)"

# `codex exec` streams progress to STDERR and prints only the final agent message to STDOUT, so
# 2>/dev/null leaves a clean channel. `--sandbox read-only` is exec's documented default; it is
# passed explicitly because this handler should never touch the filesystem. `--skip-git-repo-check`
# matters because the handler often runs from a directory that is not a git repo.
OUT="$(printf '%s' "$INPUT" | $LLM exec \
  --sandbox read-only \
  --skip-git-repo-check \
  ${SORT_CODEX_MODEL:+--model "$SORT_CODEX_MODEL"} \
  "$SYS" 2>/dev/null)" || OUT=""

#!/usr/bin/env bash
# templates/opencode/handler.sh — Sort Arena participant driven by opencode.
#
...
#
# NOT LIVE-TESTED: the `opencode` binary was not installed in the environment this template was
# written in. The invocation below follows opencode's documented `run` command
# (https://opencode.ai/docs/cli/). The parsing and validation below ARE tested — run
# `./handler.sh --selftest`. See README.md for exactly what is and isn't verified.
set -uo pipefail
...
# multi-line object would otherwise match nothing on every individual line. This matters more here
# than elsewhere: `--format default` is opencode's *formatted* output, not a raw completion.
...
# opencode's documented `run` takes the message as ARGUMENTS — no stdin piping is documented — so
# the contract and the round input are concatenated into one prompt argument. The real system
# prompt mechanism is an agent definition (`--agent`); see README.md for the file to create, which
# lets you delete most of this text and set permissions to deny tools.
SYS="You are one participant in a live sorting arena. Below is one round of play as a JSON object. Make EXACTLY ONE primitive move.
...
LLM="${SORT_LLM_CMD:-opencode}"
AGENT="${SORT_OPENCODE_AGENT:-sort-arena}"
INPUT="$(cat)"

# NOTE: opencode's `-p` is --password, NOT a prompt flag. The message is positional.
OUT="$($LLM run \
  --agent "$AGENT" \
  --format default \
  ${SORT_OPENCODE_MODEL:+--model "$SORT_OPENCODE_MODEL"} \
  "$SYS

...
if [ "${1:-}" = "--selftest" ]; then
  ...
  out="$(printf '%s' '{"round":1,"array":[3,1,2],"history":[],"budgetRemaining":10,"mode":"solo","you":"bubble-sort-claude"}' | "$PY" "$GENERATED")"
  ...
  echo "SELFTEST OK: bubble-sort-claude's generated handler emits a valid move for a real round input"
  exit 0
fi
